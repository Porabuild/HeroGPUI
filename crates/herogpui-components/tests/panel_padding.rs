//! Phase 3 overlay panel padding: an unset override keeps the stock
//! asymmetric insets, and a set value resolves both axes and feeds the same
//! padding to the resting panel and its entry-animation `ZoomBox`.
...
fn overlay_padding_resolves_both_axes_and_feeds_the_zoom_box() {
    ...
    assert!(popover.contains("let panel_padding_y = self.padding.unwrap_or(px(12.));"));
    assert!(popover.contains("let panel_padding_x = self.padding.unwrap_or(px(14.));"));
    ...
    assert!(toast.contains("let panel_padding_y = self.t.padding.unwrap_or(px(10.));"));
