...
CHECKS = [
    ...
    ('tag-group', '.tag-group', 'gap', 'tag-group field column gap',
     SRC + 'tag_group.rs',
     r'let mut root = div\(\)\.relative\(\)\.flex\(\)\.flex_col\(\)\.gap\(px\((\d+(?:\.\d*)?)\.\)\)', None),
    ('tag-group', '.tag-group__list', 'gap', 'TagGroup list gap',
     SRC + 'tag_group.rs',
     r'let mut list = div\(\)\.relative\(\)\.flex\(\)\.flex_row\(\)\.flex_wrap\(\)\.gap\(px\((\d+(?:\.\d*)?)\.\)\)', None),
    ...
    ('table', '.table__cell', 'text', 'Table cell text',
     SRC + 'table.rs',
     r'let mut table = gpui::div\([\s\S]{0,240}?\.text_size\(px\((\d+(?:\.\d*)?)\.\)\)', None),
    ...
    ('input-group', '.input-group', 'min_h', 'InputGroup height', SRC + 'input_group.rs',
     r'\.min_h\(explicit_height\.unwrap_or\(util::(FIELD_HEIGHT)\)\)', lambda _: 36.0),
    ...
    ('tabs', '.tabs__indicator', 'radius', 'Tabs selected segment -> control_radius',
     SRC + 'tabs.rs',
     r'`\.tabs__indicator` is the absolute `rounded-3xl bg-segment[\s\S]{0,400}?'
     r'\.rounded\(crate::util::(\w+_radius)\(cx\)\)', helper_px),
