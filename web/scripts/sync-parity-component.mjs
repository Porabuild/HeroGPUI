...
const COMPOSITION_RE =
  /placeholder|field_col|demo_field|fixed_demo|para\(|into_any_element|gpui::div|\.flex\(|\.gap\(|items_start|\.children\(|\.child\(|w\(px\(|variant\(|description\(|label\(|error_message|is_required|is_disabled|is_clearable|input_type|full_width|row_height|show_value|\.cols\(|\.rows\(|on_change|default_value|\.value\(|padding\(|FieldVariant|el_id|opt_usize_cb|demo_text|validate\(|\.on_selection_change|selected|select_lang|notify\(\)|\.els\(\)|\.map\(|\.iter\(|languages\(\)|virtual_names\(\)|\.into\(\)|"/;
