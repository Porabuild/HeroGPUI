...
const COLOR_AREA_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".color-area inner shadow", value: "inset 0 0 0 1px rgba(0,0,0,.1)", description: "The pinned translucent inner edge is layered with the native border so sx border-color customization remains available.", rust: "color_inner_shadow() + border(theme/sx)", status: ImplementationStatus::Implemented },
    ...
    StyleDoc { class_or_token: ".color-area__thumb shadows", value: "outer and inset 1px rgba(0,0,0,.1)", description: "The thumb carries the pinned outer and inset depth hairlines beneath its white visibility ring.", rust: "color_thumb_shadows()", status: ImplementationStatus::Implemented },
...
pub(super) const COLOR_AREA: ReferenceMetadata = ReferenceMetadata {
    ...
    version: "3.2.6",
    docs_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/apps/docs/content/docs/en/react/components/(colors)/color-area.mdx",
    api_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/react/src/components/color-area/color-area.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ColorArea.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ColorThumb.tsx",
    style_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/styles/components/color-area.css",
...
const COLOR_SLIDER_PARTS: &[PartDoc] = &[
    ...
    PartDoc {
        ...
        description:
            "Gradient spans the thumb travel and meets matching endpoint colors under one rounded clip with the alpha checkerboard. Unblurred inset shadows shade the edges without an extra outline; the thumb stays outside the clip. Independent child render state is unavailable.",
        rust_owner: "ColorSlider",
        status: ImplementationStatus::Partial,
    },
    PartDoc {
...
const COLOR_SLIDER_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".color-slider label / output", value: "text-sm font-medium tabular-nums", description: "Placement, 14px size, explicit 20px line height, medium weight, disabled label/output opacity split and tabular numeric output match.", rust: "14px/20px MEDIUM label/output row + tabular_font_features", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-slider__track horizontal / vertical", value: "20px cross-axis; total length minus 20px with 10px edge caps", description: "Cross-axis thickness, rounded total footprint, endpoint inset, pointer travel, and separate min/max cap fills match in both orientations.", rust: "track_h=20 + COLOR_SLIDER_TRACK_INSET_PX end zones as gradient stop percentages", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-slider__track shadows", value: "orientation-specific 1px inset borders", description: "Horizontal tracks shade top/bottom and vertical tracks shade left/right with the pinned translucent inset edges; the native border remains for the platform surface.", rust: "color_track_shadows(vertical) + border", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-slider alpha checkerboard", value: "repeating-conic 16px checkerboard under gradient", description: "The alpha gradient is composited over the shared 8px checker tile in both orientations.", rust: "axis-aware transparency_checker(track_h, length) + alpha gradient", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-slider__thumb", value: "size-4 rounded-2xl border-3 border-white shadow-overlay", description: "Size, circular radius, border, current-color fill, disabled default fill, and the two-layer depth shadow match.", rust: "16px + rounded(16) + border(3) + color_thumb_shadows()", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-slider__thumb transitions", value: "transform 250ms ease-out; box-shadow 150ms ease-out; reduced-motion none", description: "Thumb position and focus-ring shadow opacity interpolate through the pinned 250ms and 150ms ease-out paths; reduced motion settles both directly.", rust: "color_slider_thumb_motion + color_focus_ring_motion", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-slider__thumb[data-dragging=true]", value: "cursor-grabbing", description: "The live dragging state switches the thumb from the open-hand grab cursor to closed-hand grabbing while the track keeps its interaction cursor.", rust: "CursorStyle::OpenHand / ClosedHand", status: ImplementationStatus::Implemented },
];
...
pub(super) const COLOR_SLIDER: ReferenceMetadata = ReferenceMetadata {
    ...
    source_module: "color_picker",
    version: "3.2.6",
    docs_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/apps/docs/content/docs/en/react/components/(colors)/color-slider.mdx",
    api_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/react/src/components/color-slider/color-slider.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ColorSlider.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ColorThumb.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/Slider.tsx",
    style_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/styles/components/color-slider.css",
...
const COLOR_PICKER_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".color-picker__trigger transitions", value: "background 150ms ease-smooth; shadow 150ms ease-out", description: "The stylesheet declares no default trigger hover fill; the keyboard focus shadow now interpolates through the shared 150ms ease-out color motion and snaps under reduced motion.", rust: "color_focus_ring_motion + ring_if_focused endpoints", status: ImplementationStatus::Implemented },
    ...
    StyleDoc { class_or_token: ".color-picker__popover entering", value: "150ms ease-smooth fade-in zoom-in-95 slide 4px by resolved placement", description: "Duration, curve, fade, scale, and four-pixel translation animate together; the shared positioner feeds its resolved physical side back into the entry offset after flips and resizes.", rust: "resolved-placement handle + ZoomBox slide_x/slide_y + anim::entering_zoom(Motion::LIST_IN)", status: ImplementationStatus::Implemented },
...
pub(super) const COLOR_PICKER: ReferenceMetadata = ReferenceMetadata {
    ...
    source_module: "color_picker",
    version: "3.2.6",
    docs_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/apps/docs/content/docs/en/react/components/(colors)/color-picker.mdx",
    api_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/react/src/components/color-picker/color-picker.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ColorPicker.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/Button.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/Popover.tsx",
    style_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/styles/components/color-picker.css",
...
const COLOR_FIELD_API: &[ApiDoc] = &[
    ...
    ApiDoc { owner: "ColorField", prop: "focusRing", ty: "boolean", default: "true", description: "GPUI extension: hides only the editable field's visual focus ring while retaining focus and editing.", rust_owner: "ColorField", rust: "focus_ring(bool)", status: ImplementationStatus::Implemented },
    ...
    ApiDoc { owner: "ColorField", prop: "value", ty: "Color | null", default: "—", description: "Controlled color values may be concrete or null; null renders the documented empty field and onChange can return None while text is incomplete.", rust_owner: "ColorField", rust: "new(id, Option<PickerColor>)", status: ImplementationStatus::Implemented },
    ApiDoc { owner: "ColorField", prop: "defaultValue", ty: "Color | null", default: "—", description: "Seeds picker-owned state with a concrete color or an explicit null empty value; the outer Rust Option distinguishes omission from null.", rust_owner: "ColorField", rust: "default_value(Option<PickerColor>)", status: ImplementationStatus::Implemented },
    ...
    ApiDoc { owner: "ColorField", prop: "onBlur", ty: "(FocusEvent) => void", default: "—", description: "Focus loss commits the field like React Aria's onBlur commit: unparseable text restores the formatted last-committed value while valid text and an emptied field are untouched. The hook fires ahead of that built-in revert, the way React Aria chains the caller's onBlur ahead of commit.", rust_owner: "ColorField", rust: "on_blur(callback)", status: ImplementationStatus::Implemented },
    ...
    ApiDoc { owner: "ColorField", prop: "isRequired", ty: "boolean", default: "false", description: "Marks the label and form field required; an empty null value is treated as missing during native submission.", rust_owner: "ColorField", rust: "is_required(bool)", status: ImplementationStatus::Implemented },
    ...
    ApiDoc { owner: "ColorField.Prefix", prop: "children", ty: "ReactNode", default: "—", description: "The built-in ColorSwatch follows the current color and is omitted for the null empty state; arbitrary replacement remains unavailable.", rust_owner: "ColorField", rust: "built-in ColorSwatch", status: ImplementationStatus::Partial },
...
const COLOR_FIELD_PARTS: &[PartDoc] = &[
    ...
    PartDoc { name: "ColorField.Input", slot: "color-input-group-input", description: "State-backed text input with parsing, channel keys, wheel editing, and focus. Each simultaneously rendered editable field needs its own input::InputState identity.", rust_owner: "ColorField", status: ImplementationStatus::Implemented },
...
const COLOR_FIELD_STATES: &[StateDoc] = &[
    ...
    StateDoc { state: "Required", selector: ".color-field[data-required=true]", description: "Marks the label and reaches render state; a null or empty color fails native required validation.", rust: "is_required + FormValue::is_empty", status: ImplementationStatus::Implemented },
    ...
    StateDoc { state: "Hovered", selector: ".color-input-group[data-hovered=true]", description: "Enabled, unfocused groups interpolate the field/default hover fill over 150ms and apply the field-hover border endpoint.", rust: "hover_fade_with_duration_and_easing(150ms, EaseSmooth) + border hover", status: ImplementationStatus::Implemented },
];
...
const COLOR_FIELD_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".color-input-group transitions", value: "background/border 150ms ease-smooth; shadow 150ms ease-out", description: "Enabled, unfocused groups animate the fill through the pinned ease-smooth transition while the hover border endpoint remains immediate.", rust: "hover_fade_with_duration_and_easing(150ms, EaseSmooth) + apply_field_chrome", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-input-group hover", value: "bg-field-hover border-field-hover", description: "Enabled, unfocused groups use the field-hover or default-hover endpoint according to the variant.", rust: "hover_fade_with_duration_and_easing + border_color(field.border_hover)", status: ImplementationStatus::Implemented },
    ...
    StyleDoc { class_or_token: ".color-input-group invalid", value: "status-invalid-field bg-field-focus", description: "Danger outline/ring and the field-focus fill match both primary and secondary variants.", rust: "apply_field_chrome(invalid) + bg(field.focus/default)", status: ImplementationStatus::Implemented },
    ...
    StyleDoc { class_or_token: ".color-input-group--secondary", value: "shadow-none bg-default; hover default-hover", description: "Resting fill and shadow match, and the enabled unfocused group fades to the default-hover endpoint.", rust: "FieldVariant::Secondary + hover_fade_with_duration_and_easing", status: ImplementationStatus::Implemented },
];
...
pub(super) const COLOR_FIELD: ReferenceMetadata = ReferenceMetadata {
    ...
    source_module: "color_picker",
    version: "3.2.6",
    docs_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/apps/docs/content/docs/en/react/components/(colors)/color-field.mdx",
    api_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/react/src/components/color-field/color-field.tsx + https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/react/src/components/color-input-group/color-input-group.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ColorField.tsx",
    style_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/styles/components/color-field.css + https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/styles/components/color-input-group.css",
...
const COLOR_SWATCH_PICKER_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".color-swatch-picker__item transitions", value: "border/shadow 100ms ease-out; reduced-motion none", description: "Selection border color and focus-ring shadow opacity interpolate through the pinned 100ms ease-out path; reduced motion settles both directly.", rust: "selection-border Tween + color_focus_ring_motion", status: ImplementationStatus::Implemented },
    StyleDoc { class_or_token: ".color-swatch-picker__swatch transforms", value: "hover scale 1.1; selected scale .77; 100ms ease-out", description: "Built-in swatches interpolate hover and selected scale with selected taking precedence; caller-owned item content remains caller-owned.", rust: "swatch-scale Tween + swatch_visual_scale", status: ImplementationStatus::Implemented },
    ...
    StyleDoc { class_or_token: ".color-swatch-picker__indicator transition", value: "scale 0 to 1 over 150ms ease-out; reduced-motion none", description: "The built-in or caller indicator wrapper interpolates its selected scale over the pinned 150ms ease-out path.", rust: "indicator-scale Tween", status: ImplementationStatus::Implemented },
];
...
pub(super) const COLOR_SWATCH_PICKER: ReferenceMetadata = ReferenceMetadata {
    ...
    source_module: "color_picker",
    version: "3.2.6",
    docs_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/apps/docs/content/docs/en/react/components/(colors)/color-swatch-picker.mdx",
    api_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/react/src/components/color-swatch-picker/color-swatch-picker.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ColorSwatchPicker.tsx + https://github.com/adobe/react-spectrum/blob/react-aria-components@1.21.1/packages/react-aria-components/src/ListBox.tsx",
    style_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/styles/components/color-swatch-picker.css",
...
const COLOR_SWATCH_API: &[ApiDoc] = &[
    ...
    ApiDoc { owner: "ColorSwatch", prop: "id", ty: "string", default: "—", description: "Optional GPUI identity. A named swatch reports role=img, using color_name when supplied and hex otherwise; an unnamed preview produces no AccessKit node.", rust_owner: "ColorSwatch", rust: "id(ElementId)", status: ImplementationStatus::Implemented },
    ApiDoc { owner: "ColorSwatch", prop: "colorName", ty: "string", default: "—", description: "Overrides the generated accessible color name for a named swatch.", rust_owner: "ColorSwatch", rust: "color_name(text)", status: ImplementationStatus::Implemented },
    ...
    ApiDoc { owner: "ColorSwatch", prop: "aria-label", ty: "string", default: "—", description: "A named swatch reports role=img; color_name supplies the accessible name in this port.", rust_owner: "ColorSwatch", rust: "color_name(text) + id(ElementId)", status: ImplementationStatus::Partial },
...
const COLOR_SWATCH_STYLING: &[StyleDoc] = &[
    StyleDoc {
        ...
        description: "The default swatch is a clipped 32px circle with the theme border and HeroUI's translucent inset edge.",
        rust: "SizeXl::Md + overflow_hidden + border(layout.border_width) + color_inner_shadow()",
        status: ImplementationStatus::Implemented,
    },
    StyleDoc {
    ...
    StyleDoc {
        ...
        description: "The shared 8px checker tile sits under the translucent color layer at every swatch size.",
        rust: "transparency_checker(edge, edge) + translucent PickerColor",
        status: ImplementationStatus::Implemented,
    },
];
...
pub(super) const COLOR_SWATCH: ReferenceMetadata = ReferenceMetadata {
    ...
    source_module: "color_picker",
    version: "3.2.6",
    docs_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/apps/docs/content/docs/en/react/components/(colors)/color-swatch.mdx",
    api_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/react/src/components/color-swatch/color-swatch.tsx",
    style_source: "https://github.com/heroui-inc/heroui/blob/v3.2.6/packages/styles/components/color-swatch.css",
