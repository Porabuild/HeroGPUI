...
const POPOVER_PARTS: &[PartDoc] = &[
    ...
    PartDoc { name: "Popover.Dialog", slot: "popover-dialog", description: "Dialog focus scope and the resolved content inset (stock 12px y / 14px x, `padding(px)` overridable) are built into the monolithic panel.", rust_owner: "Popover", status: ImplementationStatus::Partial },
...
const POPOVER_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".popover__dialog", value: "p-4 outline-none", description: "The programmatic dialog focus scope matches; the resting inset resolves to 12px y / 14px x rather than v3's fixed 16px, and the `padding` builder overrides it.", rust: "panel_padding_x/y + panel_focus", status: ImplementationStatus::Partial },
...
const TOAST_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".toast", value: "absolute flex items-start gap-1.5 bg-surface px-4 py-3 shadow-overlay radius min(32px, 3xl)", description: "Card fill, row alignment, six-pixel gap, horizontal 16px padding, radius and overlay shadow match; the vertical inset resolves to 10px rather than v3's 12px, with the `padding` builder overriding both axes, the port adds no card border, and cards use flex stacking rather than absolute overlap.", rust: "ToastCardEl", status: ImplementationStatus::Partial },
