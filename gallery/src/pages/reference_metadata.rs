...
const AUTOCOMPLETE_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".autocomplete__popover", value: "trigger width; overflow hidden; bg-overlay pt-2 text-sm; radius min(32px, radius-3xl); shadow-overlay", description: "Anchors to the measured trigger with an 8px gap and 12px viewport inset, flipping when the preferred side cannot fit and the opposite side has more room; the search stays visible while the inner list scrolls and shrinks to the available height within the upstream 320px max, and virtual paging follows the visible list height. The reduced placement vocabulary and placement slide remain unported.", rust: "placement(Placement) + pt(px(8.)) + max_h(px(320.)) + scroll + overlay bg/shadow", status: ImplementationStatus::Partial },
...
const SELECT_STYLING: &[StyleDoc] = &[
    ...
    StyleDoc { class_or_token: ".select__popover", value: "min trigger width; overflow-auto; bg-overlay; radius min(32px, 3xl); shadow-overlay", description: "Anchored to the measured trigger width with an 8px gap and 12px viewport inset, scrolling within the available height; the plain list scrolls the panel and the virtual list sizes itself to the same bound. Overlay surface, radius and shadow.", rust: "scrollable_field_popover + max_h_full (plain) / Infer (virtual) + floating_radius + overlay_shadow", status: ImplementationStatus::Implemented },
...
const COLOR_PICKER_API: &[ApiDoc] = &[
    ...
    ApiDoc {
        ...
        ty: "Placement",
        default: "bottom left",
        description: "Positions and flips the floating panel through the shared placement engine, with an 8px trigger gap and 12px viewport inset.",
        rust_owner: "ColorPicker",
        rust: "placement(Placement)",
        status: ImplementationStatus::Implemented,
    },
