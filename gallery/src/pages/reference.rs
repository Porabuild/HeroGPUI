...
// Display translation
//
// The checked-in metadata deliberately mirrors the pinned HeroUI v3.2.6
...
const SCRUB_PHRASES: &[(&str, &str)] = &[
    ...
    ("The pinned React Aria", "The pinned upstream"),
    (
        "Pinned React Aria Components 1.21.1",
        "The pinned upstream 1.21.1",
    ),
    (
        "pinned React Aria Components 1.21.1",
        "the pinned upstream 1.21.1",
    ),
    ("React Aria Components 1.21.1", "the pinned upstream 1.21.1"),
...
mod tests {
    ...
    fn form_metadata_tracks_pinned_submission_contract() {
        ...
        // Exactly the pinned v3.2.6 API table — 14 rows, and v3 documents no
        ...
        assert_eq!(metadata.version, "3.2.6");
        assert!(metadata.api_source.contains("form.tsx"));
        assert!(metadata.api_source.contains("react-aria-components@1.21.1"));
        for url in [
            metadata.docs_source,
            metadata.api_source,
            metadata.style_source,
        ] {
            assert!(url.contains("/blob/v3.2.6/"));
        }
    }
...
"#;
    ...
    fn modal_metadata_tracks_compound_parts_and_full_size_parity() {
        ...
        // v3.2.6 parity: the composed close trigger accepts custom children while
    ...
    fn table_metadata_tracks_compound_parts_and_honest_gaps() {
        ...
        assert!(metadata
            .api_source
            .contains("react-aria-components@1.21.1/packages/react-aria-components/src/Table.tsx"));
        assert!(metadata
            .style_source
    ...
    fn pinned_metadata_still_records_upstream_evidence() {
        ...
        // spellings untouched so the pinned v3.2.6 contract stays auditable.
    ...
    fn display_keeps_rust_builders_and_pinned_source_links() {
        ...
        assert!(contract.cells[3].contains("/blob/v3.2.6/"));
